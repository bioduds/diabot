...
## Navigation: Glicemia is home, Nuno is a sliding overlay

[lib/glucose_chart.dart](lib/glucose_chart.dart)'s `GlucoseChartPage` is the app's permanent home screen (composed by `HomeShell` in [lib/main.dart](lib/main.dart)), not a page pushed from the chat. Only its "GLICEMIA ATUAL" reading stays pinned above the fold; the chart, legend, and template summary sit below it. Tapping **Conversar com Nuno** slides the `ChatPage` conversation up over that lower area (a chevron button retracts it back down), and the current assessment text is surfaced as Nuno's first message. The chat panel stays mounted the whole time — only its position animates — so its on-device model session is never reloaded by opening/closing it.

First-login onboarding is the one exception: the conversation opens full-screen automatically (there is no glucose data yet to show), and slides down on its own the moment the onboarding guided flow completes, revealing Glicemia as the home screen. On every later launch, with onboarding already done, Glicemia loads directly and the chat starts collapsed.
...
[lib/glucose_chart.dart](lib/glucose_chart.dart) is a read-only AGP-style view over that same local event log (manual entries and CGM auto-sync alike), and — per the section above — the app's home screen. It plots a selectable rolling window (8h–7d) with in-range shading, per-bucket averages, and a template-based (non-LLM) summary of the visible window. Pull-to-refresh and a once-a-minute timer both reload local data and re-fetch a live [lib/librelinkup.dart](lib/librelinkup.dart) snapshot (current reading, trend arrow, sensor, and patient-configured target range), merging the live reading into the plotted series so the current-reading number is always exactly the graph's last point and stays connected to the sensor line regardless of any gap in locally-synced history. Press-and-hold on the chart draws a vertical dashed line from the touched point to the top of the chart with the glucose value labeled there, colored by whether it falls inside the target range. A settings screen reached through the app-bar gear icon holds the period and source (`manual`/`cgm`/`all`) filters, keeping the main chart on one screen without scrolling.

### Physiological glucose estimate (Kalman filter)

[lib/cgm/kalman_filter.dart](lib/cgm/kalman_filter.dart) is a small, CGM-agnostic 2-state (value, velocity) linear Kalman filter. [lib/cgm/glucose_estimator.dart](lib/cgm/glucose_estimator.dart) wraps it with glucose-specific defaults to turn each reading into an `estimatedNow` that partially compensates the sensor's own ~10-minute interstitial lag, alongside the untouched `observed` value, a 0-1 confidence, and the current rate of change. It never predicts from readings it hasn't received yet — it only compensates a delay every CGM already has relative to blood glucose.

The chart header always shows both numbers side by side, "SENSOR" (raw) and "ATUAL" (the Kalman estimate) — neither is ever hidden or gated behind a confidence threshold. "ATUAL" carries a color-coded confidence indicator and how many minutes ahead of the sensor it compensates; its info icon explains the method in plain language. The chart itself extends the estimate with a dashed +5/+10/+15-minute forecast, a constant-velocity extrapolation of the filter's own linear state, not an independent predictive model.
